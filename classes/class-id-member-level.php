...
class ID_Member_Level {
	...
	function add_level($level) {
		...
		$sql = $wpdb->prepare('INSERT INTO '.$wpdb->prefix.'memberdeck_levels (product_type, level_name, level_price, credit_value, txn_type, level_type, recurring_type, limit_term, term_length, plan, license_count, enable_renewals, renewal_price, enable_multiples, combined_product) VALUES (%s, %s, %s, %d, %s, %s, %s, %d, %d, %s, %d, %d, %s, %d, %d)', $this->product_type, $this->level_name, $this->level_price, $this->credit_value, $this->txn_type, $this->level_type, $this->recurring_type, $this->limit_term, $this->term_length, $this->plan, $this->license_count, $this->enable_renewals, $this->renewal_price, $this->enable_multiples, $this->combined_product);
		$res = $wpdb->query($sql);
	...
	public static function update_level($level) {
		...
		$sql = $wpdb->prepare('UPDATE '.$wpdb->prefix.'memberdeck_levels SET product_type = %s, level_name=%s, level_price=%s, credit_value = %d, txn_type=%s, level_type=%s, recurring_type=%s, limit_term = %d, term_length = %d, plan=%s, license_count=%d, enable_renewals = %d, renewal_price = %s, enable_multiples = %d, combined_product = %d WHERE id=%d', $product_type, $level_name, $level_price, $credit_value, $txn_type, $level_type, $recurring_type, $limit_term, $term_length, $plan, $license_count, $enable_renewals, $renewal_price, $enable_multiples, $combined_product, $level_id);
		$res = $wpdb->query($sql);
	}
