...
/**
 * Function for checking that any of the boxes (Stripe Connect, Balanced, PayPal Creator, Mailchimp) on Creator Settings (?payment_settings=1) is enabled
	...
	foreach ($levels as $level) {
		if ($i + 1 <= (count($levels) - $new_levels)) {
			...
			if ($level_type == 'recurring') {
				$args['plan'] = str_replace(' ','-',strtolower($args['level_name']));
			}
			$args['license_count'] = 0;
			$args['limit_term'] = 0;
			$args['term_length'] = '';
