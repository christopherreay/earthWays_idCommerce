...
function memberdeck_styles() {
	...
	if (!empty($settings)) {
		...
		if (is_array($settings)) {
			...
			global $post;
			if (isset($post)) {
				if (has_shortcode($post->post_content, 'idc_checkout') || has_shortcode($post->post_content, 'memberdeck_checkout') || isset($_GET['mdid_checkout']) || isset($_GET['idc_renew'])) {
					if ($es == '1') {
...
function memberdeck_disable_others() {
	$get_array = array('payment_settings', 'backer_profile', 'edit-profile', 'creator_projects', 'mdid_checkout', 'idc_orders', 'key_valid');
